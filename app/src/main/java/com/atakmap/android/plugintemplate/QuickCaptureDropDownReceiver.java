...
public class QuickCaptureDropDownReceiver extends DropDownReceiver implements OnStateListener {
    ...
    /** layerUrl → whether the layer has attachments enabled (populated after project load). */
    private final Map<String, Boolean> layerHasAttachments = new LinkedHashMap<>();
    ...
    public QuickCaptureDropDownReceiver(MapView mapView, Context pluginContext) {
        super(mapView);
    ...
    private void checkAttachmentSupport() {
        if (project == null) return;
        layerHasAttachments.clear();
        ...
        worker.execute(() -> {
            for (QuickCaptureProject.Template t : proj.templates) {
                ...
                String layerUrl = proj.urlFor(t);
                if (layerUrl == null || layerHasAttachments.containsKey(layerUrl)) continue;
                boolean has = client.fetchHasAttachments(layerUrl);
                layerHasAttachments.put(layerUrl, has);
            }
        });
    }
    ...
    private void capture(QuickCaptureProject.Template template) {
        ...
        if (inputs.isEmpty()) {
            maybeShowPhotoPicker(template, new LinkedHashMap<>(projectInputValues));
            return;
        }
        showInputDialog(template.label, inputs, values -> {
            Map<String, Object> merged = new LinkedHashMap<>(projectInputValues);
            merged.putAll(values);
            maybeShowPhotoPicker(template, merged);
        });
    }

    private void maybeShowPhotoPicker(QuickCaptureProject.Template template,
                                      Map<String, Object> inputs) {
        String layerUrl = project != null ? project.urlFor(template) : null;
        boolean supportsAttachments = Boolean.TRUE.equals(layerHasAttachments.get(layerUrl));
        if (!supportsAttachments) {
            submit(template, inputs, Collections.emptyList());
            return;
        }
        new PhotoPickerDialog(getMapView().getContext(), pluginContext, template.label,
                photos -> submit(template, inputs, photos)).show();
    }
    ...
    private void submit(QuickCaptureProject.Template template, Map<String, Object> inputs,
        ...
        submit(template, inputs, photos, point);
    }

    private void submit(QuickCaptureProject.Template template, Map<String, Object> inputs,
                        List<Uri> photos, GeoPoint point) {
        ...
        final String token = tokenEdit.getText().toString().trim();
        final Context appContext = getMapView().getContext().getApplicationContext();
        worker.execute(() -> {
            try {
                ...
                // Upload photos as attachments if the feature was created successfully
                int uploadedCount = 0;
                if (objectId >= 0 && !photos.isEmpty()) {
                    for (Uri photo : photos) {
                        try {
                            boolean ok = client.addAttachment(layerUrl, token, objectId, photo, appContext);
                            if (ok) uploadedCount++;
                        } catch (Exception attachErr) {
                            // Individual attachment failure — don't abort the rest
                        } finally {
                            PhotoAttachmentManager.deleteTempFile(photo);
                        }
                    }
                }

                final int attached = uploadedCount;
                final int attempted = photos.size();
                getMapView().post(() -> {
                    ...
                    if (attempted > 0) {
                        toast(template.label + " captured · " + attached + "/" + attempted + " photo(s) uploaded");
                    } else {
    ...
    private void beginPlacement(QuickCaptureProject.Template template, Map<String, Object> inputs) {
        ...
        placementListener = event -> {
            ...
            cancelPlacement();
            if (point != null && point.isValid()) submit(template, inputs, Collections.emptyList(), point);
        };
